...
form.addEventListener("submit",function (event){
    ...
    if(sname.value.length < 2){
        ...
        let fmessage = document.querySelector("#sname_text")
    ...
    if(email.value.length < 8){
        ...
        emessage.innerText = "password must be graeter than 8 characters";
    ...
    if(password.value.length < 8 && password.value.includes(' ')){
    
        let pmessage = document.querySelector("#pass_text")
    
        pmessage.style.display = "block";
    
        pmessage.innerText = "<li>password must be graeter than 8 characters</li>";
